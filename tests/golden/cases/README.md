...
| `grep_jar_artifact` | ③ | src 内 jar を `walk_skipped_binary` で除外・`Binary file … matches` 行を `bad_grep_line` 化。正常 hit のみ TSV 化 | `diagnostics_summary.txt` あり。jar は `scripts` 内 zipfile で `date_time` 固定の決定的バイト列。`.gitattributes` で `binary` 指定 |
...
| `messy_c_legacy` | ① | 乱雑 C（タブ/深ネスト/多文1行/コメントアウト/ブロックコメント）の分類と snippet サニタイズ（タブ→空白）。本セットの分類: 1=その他・3=比較・4=宣言・7=その他 | R3: 分類は現挙動を凍結 |
