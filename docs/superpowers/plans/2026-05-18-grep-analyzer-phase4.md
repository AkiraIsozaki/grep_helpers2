...
**本計画の埋め込みコードは `/workspaces/grep_helpers2` 実環境（py-tree-sitter 0.21・tree-sitter-java/c）で実行検証済**（sort_key/_sanitize/clamp_lines/heuristic_span/ts_span/_paren_span/exec_spans/build_snippet の全アサーション green・3観点AIレビュー収束）。逸脱が必要なら spec 本体（ユーザ所有）を先に改訂する。
...
各ケース `input/<kw>.grep`・`src/<file>`・`expected/<kw>.tsv`（先頭 BOM＋ヘッダ行は既存 golden と同形式）。**全新規ケースは単一 part（`max_rows_per_part` 未満。既定 ≫ 数行なので自動的に単一）、cp932 ラウンドトリップ不可逆文字を含めない、かつ src 本文に `{SOURCE_ROOT}` 解決後の絶対 src パス文字列を含めない（行単位逆置換の snippet 誤置換回避＝§11(b)）**（`cp932_resume` 含む。`txt.encode("cp932").decode("cp932")==txt` を Step2 で確認）。手計算の要点（snippet 列・確定値・実環境実行で検証済）:
