...
def _has_err(node) -> bool:
    cur = node
    while cur is not None:
        if cur.type == "ERROR" or cur.is_missing or cur.has_error:
            return True
        cur = cur.parent
    return False
...
def ts_span(language: str, file_text: str, lineno: int):
    ...
    粒度表へ昇格／block 等到達は直近 statement／無ければ None）。ERROR/
    MISSING を祖先まで含むなら None。
    """
    ...
    while cur is not None:
        ...
        if t in _BLOCK:
            return ((last_stmt.start_point[0], last_stmt.end_point[0])
                    if last_stmt is not None else None)
        cur = cur.parent
    return ((last_stmt.start_point[0], last_stmt.end_point[0])
            if last_stmt is not None else None)
```

- [ ] **Step 5: Run & commit** — `python -m pytest tests/unit/test_snippet.py -q` → PASS。tree-sitter 版差でノード型が異なる場合のみ `python -c "import tree_sitter_java,tree_sitter_c"` で確認し `_GRAN_*` を実型へ調整（spec §9 粒度表の意味は維持）。
...
def build_snippet(language: str, dialect: str, file_text: str,
    ...
    java/c: ts_span→無ければ heuristic→ヒット1行（フォールバック順）。
    proc: EXEC 区間は原ソース span／区間外は c と同経路→heuristic→1行。
    sql/shell: heuristic_span。連結前に各行へ規約①_sanitize→②_escape_sep
    を適用し clamp_lines（spec §9）。
    """
...
from grep_analyzer.snippet import build_snippet
```
`fixedpoint.py:333-338` の `Hit(...)` を変更（`file=c.relpath` は**不変**、`snippet` のみ差替。`text,language,dialect` は直上 323 行で取得済）:
...
def test_合成ケースのTSVが期待値と完全一致する(golden_case, tmp_path):
    ...
    for expected in sorted((golden_case / "expected").glob("*.tsv")):
        ...
        actual = actual.replace(sr + "/", "{SOURCE_ROOT}/", 1)  # §11(b)
    ...
    if cfg.is_file() and json.loads(
        ...
        for expected in (golden_case / "expected").glob("*.tsv"):  # §11(d)
...
各ケース `input/<kw>.grep`・`src/<file>`・`expected/<kw>.tsv`（先頭 BOM＋ヘッダ行は既存 golden と同形式）。手計算の要点（snippet 列・確定値）:

- `multiline_if/`：`src/A.java`= `class A{ void m(int s){\n  if (s\n      == 1) {\n    g();\n  }\n}}\n`、`input` ヒット行=2。期待 snippet=`  if (s \n       == 1)`（条件部のみ・本体非包含＝ts_span (1,2)）。
...
- `parsefail_fallback/`：`src/B.java`= `@@@@@\n`、ヒット=1。期待 snippet=`@@@@@`（ts_span None→heuristic→1行）。
