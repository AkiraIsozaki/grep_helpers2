...
- [ ] **Step 4: テストが通ることを確認（当該ファイル＋全体回帰ゼロ）**

Run: `python -m pytest tests/unit/test_dispatch.py -q`
Expected: PASS（既存 dispatch テスト＋新規4本）

Run: `python -m pytest -q`
Expected: 全 PASS・**0 failed / 0 error**（既存43に対し回帰ゼロ）。赤があれば commit せず Task 内で修正する。
...
> **既存テストの契約変更（必読）**: spec §5.1 手順4 は EXEC SQL ヒューリスティックを「`.c`/`.h`/`.pc`/**未知**」に適用すると規定する（v7 で明文化）。Phase 1 実装は未知拡張子経路で内容を一切見なかったが、本 Task で未知拡張子も手順3（シェバン）→手順4（EXEC SQL）→手順5（c）を通るようになる。既存テスト `test_判定不能はcにフォールバックする` の入力 `"no exec sql here"` は正規表現 `\bEXEC\s+SQL\b`(IGNORECASE) に**実際にマッチする**（"exec sql" を含む）。Phase 1 では未知拡張子が内容を見なかったため `c` を返していたが、新仕様では `proc` を返す。これは回帰ではなく **spec §5.1 手順4 への意図的整合**。よって既存テストの入力を「真に EXEC SQL を含まない文字列」へ更新し、新挙動は別テストで TDD する（反射的なグリーン化ではなく、契約変更に伴うフィクスチャ修正であることを履歴に残す）。

- [ ] **Step 1: 既存テストのフィクスチャを契約変更に合わせて更新する**

`tests/unit/test_dispatch.py` の既存 `test_判定不能はcにフォールバックする` を次に書き換える（入力文字列のみ変更。意図＝「判定不能は c」は不変、フィクスチャが偶然 "exec sql" を含んでいた誤りを是正）:
```python
...
Run: `python -m pytest tests/unit/test_dispatch.py -q`
Expected: FAIL（`.csh` が `_EXT_MAP` 未登録で `"c"` 判定／`ImportError: cannot import name 'extension_resolves_language'`／拡張子なしシェバン未対応）
...
同ファイルの末尾に `extension_resolves_language` を追記（spec §5.1 手順1〜2 で言語が確定するか＝手順3 シェバン検出に到達しないかの述語。Task 7 の `unsupported_shebang` を手順3 の範囲に限定するために使う）:
```python
...
- [ ] **Step 5: テストが通ることを確認（当該ファイル＋全体回帰ゼロ）**

Run: `python -m pytest tests/unit/test_dispatch.py -q`
Expected: PASS（更新した `test_判定不能はcにフォールバックする` ＋ Task1/Task2 追加分すべて緑）

Run: `python -m pytest -q`
Expected: 全 PASS・**0 failed / 0 error**（既存43に対し回帰ゼロ。`detect_language` 書き換えは dispatch/pipeline/golden 横断のため全体ゲート必須）。赤があれば commit せず Task 内で修正する。

- [ ] **Step 6: コミット**

```bash
git add src/grep_analyzer/dispatch.py tests/unit/test_dispatch.py
git commit -m "feat(dispatch): .csh/.tcsh・シェバン判別・extension_resolves_language（spec §5.1）"
```
...
Run: `python -m pytest -q`
Expected: 全 PASS・**0 failed / 0 error**（回帰ゼロ）。赤があれば commit せず Task 内で修正する。
...
Run: `python -m pytest -q`
Expected: 全 PASS・**0 failed / 0 error**（`_SQL_RULES` 差し替えは golden `sql_direct` 横断のため全体ゲート必須。既存 `sql_direct` が `比較/medium` のまま不変であることを含む）。赤があれば commit せず Task 内で修正する。
...
Run: `python -m pytest -q`
Expected: 全 PASS・**0 failed / 0 error**（既定 bourne で既存 `shell_direct` golden・pipeline 不変）。赤があれば commit せず Task 内で修正する。
...
Run: `python -m pytest tests/unit/test_chase.py -q`
Expected: PASS（境界テスト含め全緑）

Run: `python -m pytest -q`
Expected: 全 PASS・**0 failed / 0 error**（`chase.py` は新規・非配線だが全体ゲートで回帰ゼロを確認）。赤があれば commit せず Task 内で修正する。
...
def test_拡張子なし非シェルシェバンはunsupported_shebangを診断に出す(tmp_path: Path):
    src = tmp_path / "src"
    src.mkdir()
...
Expected: PASS（新規 integration 4本＋既存 pipeline unit すべて緑。負の契約 `test_既知拡張子の非シェルシェバンは診断しない` 含む）

Run: `python -m pytest -q`
Expected: 全 PASS・**0 failed / 0 error**（pipeline 全置換は golden 全件横断のため全体ゲート必須。既存6 golden 不変を含む）。赤があれば commit せず Task 内で修正する。
...
Expected: 既存6ケース PASS。`tests/golden/conftest.py` は `cases/*/` を無条件 parametrize するが、新規ケースは Step 2 でディレクトリを作るまで存在しない。なお `test_golden.py` は `expected/*.tsv` が無いケースでは照合0件で `rc==0` のみ検証し**素通り PASS** する仕様のため、Step 2 でケース骨格を作っても Step 3 で `expected/*.tsv` を配置するまでは「偽の緑」になり得る。実質検証は **Step 4**（expected 配置後）で行う。
