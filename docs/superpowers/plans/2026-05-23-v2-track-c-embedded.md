...
git commit -m "feat(embed): extract_jsp_java 逆マスク（JSP scriptlet/式/宣言/EL を残し他を行数保存で空白化）"
```
...
def test_jsp_snippet_多行scriptletはregion span():
    src = "<html>\n<%\n  int x = 1;\n%>\n</html>\n"
...
`tests/golden/cases/html_passthrough/src/p.html`:
...
`tests/golden/cases/angular_chain/src/app.component.html`（direct＋`*ngFor` 由来の多ホップ）:

```html
<ul>
  <li *ngFor="let row of TRACKED">{{ row.code }}</li>
</ul>
<p [title]="TRACKED.length">count</p>
```

`tests/golden/cases/angular_chain/input/TRACKED.grep`:

```
app.component.html:2:  <li *ngFor="let row of TRACKED">{{ row.code }}</li>
app.component.html:4:<p [title]="TRACKED.length">count</p>
```
