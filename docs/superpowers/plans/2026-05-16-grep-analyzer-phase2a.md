...
> **改訂履歴:** v1（`docs(plan)`）→ v2（1巡目3並行レビュー反映 `fix(plan)`）→ **v3（本書・2巡目3並行レビュー反映）**。2巡目の Critical/High を spec/コードで技術検証し是正:
...
- **マスク非対称（抽出=マスク／automaton 走査=raw）の根拠**: 追跡シンボルの**抽出**は `mask_literals` 後（文字列内 `"x=1"` をシンボル化しない＝spec §8 偽陽性ノイズ抑止）。一方、既存追跡シンボルの**出現検出**（automaton scan）は raw 行で行う。Shell/SQL の変数利用は `"$CODE"`/`'...&v...'` 等**文字列内に正当出現**し（spec §7 は Shell/SQL の文字列内変数参照を分類対象とする）、走査をマスクすると実利用を取りこぼすため。Java/C の文字列/コメント内言及がヒット化する点は §8.4「実行時の文字列連結・整形で生成される名前」と同類の字句ツール既知近似（confidence は分類器準拠・人間判断＝§8.4 正本）。本非対称は意図的設計として明記する。
...
def test_C定義とconst定数を抽出しProCホスト変数は追跡投入しない():
    ...
    # spec §8.1 手順1: Pro*C ホスト変数 :host は外部・ホスト境界＝追跡投入しない
...
Run: `python -m pytest tests/unit/test_chase.py -q` → PASS（既存7＋新規7）
...
def run_fixedpoint(
    ...
    def _ingest(parent: Occurrence, language: str, dialect: str, line: str, hop: int):
    ...
    for s in seed_hits:
        ...
        _ingest(occ, lang, dia, s.snippet, hop=1)
...
> **実装注（次レビュー重点）:** エッジは `intro[s]`（s を実際に抽出した parent_occ 群）→ 発見 child のみ。seed 行が s を抽出した場合のみ当該 seed occ が親（全 seed 無条件親化＝v1/v2 偽 chain を根治）。hop≥2 は s を抽出した child occ が親。`provenance` の追跡シンボル単純パス（spec §9・07e81bb）が定数循環 A→B→A を `prov_cycle_cut` で打ち切り＝有限・決定的。`seen` は Occurrence 単位（異なる単純パスは `chains_to` が経路別 chain＝spec §9 重複許容）。`is_seed_location` で seed 物理行を間接除外（keyword≠シンボル名でも seed 行を再出力しない）。`_file_meta` 単一デコード＋`line_cache`/`meta_of`/`raw_of` キャッシュで再 I/O ゼロ（三重デコード解消）。`hop>max_depth` 打ち切りは `prov_max_depth` 記録（dedup・spec §8.1手順6）＝Task10 が `--max-depth` で必ず観測可能。
...
> - **不変（8件）**: java_direct（STATUS_OK 出現は seed 行 2/4 のみ＝is_seed_location 除外）、c_direct（CODE は seed 行のみ）、proc_direct（`p` は too_short）、sql_direct（chase 空）、csh_direct/csh_noext_shebang/sh_noext_shebang（CODE 出現は seed 行のみ）、encoding_utf8（`コード` は ASCII 開始でなく抽出されない＝v1 既知の ASCII 識別子限界）。
...
`chain_multipath/src/D.java`: `class D { int use = HUB; }`
`chain_multipath/input/KSEED.grep`: `A.java:1:static final int KSEED = 9;`
（KSEED→(B行で HUB 抽出)→HUB、KSEED→(C行で HUB 抽出)→HUB の 2 経路で `HUB@D.java:1` へ到達＝2 chain 行。**B.java:1/C.java:1 の HUB 定義行も `indirect:constant`（HUB は KSEED 経由で発見）として各 2 行出る点も決定的に正しい**＝Step3 で全行を目視確定する）
...
- `chain_multipath`: direct `A.java:1`。`HUB@D.java:1` が **chain 2 行**: `KSEED@A.java:1 -> KSEED@B.java:1 -> HUB@D.java:1` と `KSEED@A.java:1 -> KSEED@C.java:1 -> HUB@D.java:1`（決定的順）。加えて `HUB@B.java:1`/`HUB@C.java:1`（HUB 定義行・KSEED 経由 indirect:constant）も各 2 chain 行、`KSEED@B.java:1`/`KSEED@C.java:1`（KSEED の出現・indirect:constant）も各 1 行。これら全行が決定的＝**spec §11「chain 複数経路の決定性」を `HUB@D.java:1` の 2 行で固定**（他行は付随する正しい間接ヒット）。
- `getter_no_expand`: direct `S.java:1`（ただし keyword=`vv` len2≥2＝seed 自身は direct）。`T.java:1`/`T.java:2` に `getName` が `indirect:getter`・`confidence=low`（chain `vv@S.java:1 -> getName@T.java:1` 等）、`getName` から先のシンボルは出ない（横展開なし）。
...
> 次レビュー重点候補: ① intro エッジで複数 seed が同一 sym を別箇所導入する場合の chain 増（spec §9「重複許容」と整合だが Task11 期待 TSV を実生成・全行目視で確定する手順を Task11 Step3 に明記済）② `chain_multipath` の付随行（HUB@B/C・KSEED@B/C）が全て決定的に正しいことの目視 ③ getter terminal が hop≥2 で多数発見された場合の報告量（spec §8.4 全件は満たすが大規模時の degrade は 2b・境界に明記）。
