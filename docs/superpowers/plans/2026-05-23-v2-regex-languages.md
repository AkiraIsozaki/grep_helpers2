...
Expected: 全 PASS（現状の収集数は 259＝`python -m pytest --co -q | tail -1` で確認。green 件数を実行して記録する）。この件数を Inv-B 基準とし、以降のタスクで既存テストの赤化が無いことを確認する基準とする。
...
_SQL_KW = frozenset(
    ...
    # Oracle データ型（型名 var 抽出のすり抜け対策・設計 §5.2 二次防御）。
    # admit は大小区別の完全一致（stoplist.py:65-69）＝PL/SQL 慣例の大文字形で登録する。
    # 主防御は Task 4 の抽出是正（型名を抽出しない）。小文字型名は本網では拾わない既知限界。
...
Run: `python -m pytest tests/unit/test_chase.py -q`
Expected: PASS（新規 PLSQL＋既存の Inv-B 既知境界＝複数代入 `a:=1;b:=2`→`["a","b"]`・トリガ相関名 `:new.col`/`:old.col`→`["col"]`・`rec.field`→`["field"]`・バインド `:bind`/置換 `&subst`→`[]`・`v_code := 'X';`→`["v_code"]` すべて不変）。
...
- Test: `tests/unit/test_classify.py`（**既存ファイルへ追記**＝既存 `classify_hit` テストと docstring を温存。上書きしない）

- [ ] **Step 1: 失敗するテストを書く（既存ファイルへ追記）**

`tests/unit/test_classify.py` は既存（`classify_hit` の2テストと module docstring あり）。**ファイル先頭の import 群**に次を追加:

```python
from grep_analyzer.classifiers.regex_classifier import classify_groovy, classify_perl
```

**ファイル末尾**に次のテスト関数を追記（docstring は再掲しない＝既存を温存）:
...
- [ ] **Step 1: 失敗するテストを書く（既存 test_classify.py へ追記）**

`tests/unit/test_classify.py` 先頭の既存 import（Task 5 で追加した `from grep_analyzer.classifiers.regex_classifier import classify_groovy, classify_perl`）へ `classify_sql` を加える:
...
- 既存 `test_拡張子なし非シェルシェバンはshellにしない`（68-70行）を**関数ごと削除し**、次の新関数群へ**置換**する（旧関数を残すと `== "c"` assert が perl 昇格で赤化＝Inv-B 違反になるため、追記でなく置換）。なお `test_非シェルシェバンはotherを返す`（45-47行・`shebang_dialect("#!/usr/bin/perl")=="other"`）は `shebang_dialect` 不変のため**そのまま温存**（perl が "other" を返すのは設計どおり）:
...
**必須（設計 §3.2）**: `shebang_dialect` と `shebang_language` は**同一の第1物理行解釈・env/版剥がし規則を共有する**。`shebang_dialect` を `_shebang_interp`（版剥がし済 interpreter）経由へリファクタし、`bourne`/`cshell`/`other`/`None` の戻り値契約は不変に保つ（perl/python 等→`"other"`、シェバン無→`None`、sh/bash/ksh/dash→`bourne`、csh/tcsh→`cshell`）。これにより両関数の不整合を構造的に排除する。既存 `test_dispatch.py` の `shebang_dialect` 系テスト（34-52行）が全 green であることで契約不変を確認する。
...
同じ Step 1-4 の手順で（chaser は代入の**左辺のみ**抽出し RHS の関数名は追わない＝既存 `indirect_var_shell` と同型の単一シンボル伝播）:
- `perl_indirect_var`: keyword は値（例 `X`）。src の `my $CODE = "X";` で X が direct ヒットしつつ `CODE` を chase var として抽出 → 別行 `print "$CODE";`（または別ファイル）の `$CODE` が **indirect:var（via_symbol=CODE）**として固定される。input.grep は X の direct 行のみ（CODE 経由は fixedpoint が発見）。
- `groovy_direct`: `def CODE = "X"` ＋ `println CODE`。
- `groovy_indirect_const`: keyword=値。`final CODE = "X"` で CODE を constant 抽出 → 別箇所の `CODE` 参照が **indirect:constant**。
- `plsql_proc`: `.pkb` で `PROCEDURE`/`v_x NUMBER := 'X';`/`DBMS_OUTPUT.PUT_LINE('X')` を含み、宣言/代入/出力 category と「型名 NUMBER を var 抽出しない」を固定。
...
- [ ] **Step 3: Inv-B 補完ゲート（diff 死角＝既存 chase 抽出能力）を確認**

Run: `python -m pytest tests/unit/test_chase.py -q`
Expected: PASS（golden に現れない既存抽出能力を unit で守る＝複数代入 `a:=1;b:=2`→`["a","b"]`・トリガ相関名 `:new.col`/`:old.col`→`["col"]`・`rec.field`→`["field"]`・バインド/置換→`[]` がすべて温存）。
