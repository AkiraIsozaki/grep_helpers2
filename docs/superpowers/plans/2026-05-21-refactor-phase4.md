...
## R-1 判定記録（Phase 4 V5 観点 B Minor 反映）

**`patterns/literal_masking.py` の java/c/proc 重複の集約判定**:
- 実機 `MASK_SPECS` の `java` / `c` / `proc` キーは **byte 完全同一**の regex 集合（文字列リテラル `'...'` `"..."`、行コメント `//...`、ブロックコメント `/*...*/`）
- 設計 §5 R1 三判定: (1) 同一知識=Yes（C 系リテラル/コメント文法を共有、Pro*C は EXEC SQL を別途処理）、(2) 同一用語で説明可=Yes（「C 系リテラルマスク」）、(3) Rule of Three=3 箇所で集約条件充足
- **判定: 集約候補。ただし Pro*C の将来個別化（EXEC SQL ホスト変数の特殊扱い等）の可能性があるため、Phase 4 では実装変更を見送る。** 集約する場合は `_C_FAMILY_MASK_SPECS` 等の共通定数に寄せ、`MASK_SPECS = {"java": _C_FAMILY, "c": _C_FAMILY, "proc": _C_FAMILY, ...}` で参照する形（将来フェーズで再検討）
...
- `patterns/literal_masking.py` の java/c/proc 集約（上記 R-1 判定参照、Pro*C 個別化のタイミングと連動）
