...
def test_classify_hit_TSコード行はtypescript不変():
    # L4（items = TRACKED） → typescript の代入
    assert classify_hit("typescript", "", _C, 4, "")[0] == "代入"
...
def test_build_snippet_inline_template行は1行():
    ...
    assert "@Component" not in out                # template_string 巨大スパンにならない
```
...
```
`_scan_file` の `if automaton_obj is not None:` ブロック（現 43-56行）を次へ置換:
```python
