...
- 日付: 2026-05-30（rev.4＝批判的レビュー第3ラウンド反映）
...
- **union の cap/nchunks レイヤ（新規・一意確定）**: `apply_global_cap`/`maybe_spill` は per-keyword state のまま。union 走査の `compute_nchunks` は **ロックステップ専用のグローバル予算で一意に**決める＝`n_live = |union 記号|`、`n_intro = Σ_kw len(state.introducers)`、`in_memory_len = Σ_kw state.edge_store.in_memory_len()`、予算は `--memory-limit` 全体。二択併記を排し集合演算で確定（決定的）。automaton 単調性により nchunks 分割は byte 透過（chunk 同居で他 keyword 記号が混じっても absorb で除外）。`automaton_split` 診断は §6.2 のとおり**グローバル hop 単位で共有1回**記録。
...
**v1 スコープ前提（C1 を構造的に閉じる・rev.4）**: 対象言語は Java/C/SQL/Shell（spec §2.3）で、実コーパス encoding は cp932/euc-jp/latin-1/utf-8＝**すべて ASCII 透過**。**非ASCII透過 encoding（UTF-16/32・EBCDIC 系等、BOM/NUL を持たないものを含む）はソースとして想定外＝spec 前提条件で除外**し、`--encoding-fallback`/`--lang-map` に混ぜる運用は prefilter 上位集合保証の対象外（§8.4 明記）。これにより C1 は「想定外 encoding」の限界に縮約され、現実コーパスでは発生しない。

**rev.3 の防御条件（反転＝enc-memo 非依存・デコード前判定）**: 前提に加えた保険として、危険な **非ASCII透過 encoding のファイル**は **walk 段で生バイトからデコード前に判定**して保留する。
...
- **残存限界（§8.4 明記）**: 「BOM/NUL いずれも持たない非ASCII透過 encoding 全般」（BOM 無し UTF-16/32＋EBCDIC 系等）は walk 段検出を逃れ得る。上の v1 スコープ前提で**想定外として除外**するのが一次防御で、walk 段 BOM/NUL 判定は二次防御。回帰テストに「BOM 付き UTF-16・先頭 CJK・後方 ASCII 識別子」を投入し BOM 経路を固定。
- **read サイズ拡張の診断影響**: `_is_binary` の read を 8KiB→64KiB に拡張すると、64KiB 内に NUL を持つファイルの `walk_skipped_binary` 判定が増え得る（TSV 不変・診断のみ変動）。§8.4 に明記。
- 既存安全弁 `ripgrep.py:41`（非ASCII記号を含む hop は prefilter 全無効）はそのまま維持。
- **候補爆発リスク（rev.4・性能と決定性の交点）**: rg は `-F` **部分文字列**マッチだが automaton は**語境界一致**。ありふれた短い識別子（`id`/`get` 等）が union に入ると rg 候補がほぼ全 60GB に膨張し初回 hop デコードが事実上全走査化する。緩和: ① `min_specificity`/stoplist が partition でありふれ記号を chase 対象から落とす（union 縮小）、② rg を **`-w`（語境界）** で呼ぶ案＝automaton 語境界に近づけ偽陽性を激減（ただし rg の語境界定義 ⊇ `_IDENT`＝automaton がヒットするファイルを rg が必ず含む「上位集合」である証明が必要。崩れると Inv-3 違反のため、安全性証明が取れるまでは `-F` を既定、`-w` は要検証の最適化）。この爆発は §8/§9 のプローブで定量化する。

### 4.5 rg 同梱と解決（オフライン・rev.2 で C-1〜C-3 反映）

- **配置はパッケージ内**: `src/grep_analyzer/vendor/ripgrep/{aarch64,x86_64}/rg`（ルート `vendor/` は non-editable install/wheel で消えるため不可）。`pyproject.toml` の `[tool.setuptools.package-data]` で同梱、解決は **`importlib.resources`**（`__file__` 相対は zip-safe でない）。**サポートする配備モデルはディレクトリインストール**（オフライン wheelhouse → `pip install` → 展開済み `site-packages`）であり、`importlib.resources.files()` が**実ファイルシステム上の実体パス**を返す前提に立つ。zip-import インストールのように実体パスを持たない Traversable では `_vendored_rg_path()` の `.is_file()` が例外を投げ→捕捉→**None に安全縮退**（→ PATH/None フォールバック）する＝同梱 rg を使わないだけでクラッシュしない。`as_file()` での実体化や module-global な `ExitStack` 保持は本配備モデルでは不要（YAGNI）ゆえ採らない。
...
- **ロックステップによる診断順序の保全（必須）**: 現状 `diag` は run で1個（`pipeline.py:41`）。ロックステップで全 keyword の `absorb_results`/`ingest_one` を hop ごとに回すと、`decode_replaced`/`symbol_rejected`/`prov_max_depth` 等が **keyword インターリーブ順**で追記され、逐次版の diagnostics と順序が割れる（§10.3 順序契約・既存 diagnostics golden を割る恐れ）。→ **keyword ごとに専用 `Diagnostics` を持たせ、最後に決定的順序でマージ**して逐次版と同一順序を再現する。
- **マージ順の一意確定（rev.4）**: 逐次版は「keyword sorted 順に、各 keyword の (walk は無関与) → direct → indirect を順に追記」。再現するマージ順は **(1) 共有 walk 診断（`collect_files` の1回分）→ (2) keyword sorted 順に各 keyword の direct 診断 → (3) keyword sorted 順に各 keyword の indirect 診断**。各カテゴリ内の detail はこの連結順で並び、`render` のカテゴリ sorted（`diagnostics.py`）と合わせて逐次版とバイト一致。`*_logged`（`replaced_logged` 等）は ChaseState ローカル＝keyword 別 Diagnostics と 1:1 対応させ、共有 diag に別 keyword の重複抑止状態を混ぜない。
- **`automaton_split` の所属（§4.1 と整合）**: union 走査は**グローバル hop 単位で1回**なので `automaton_split` も**共有診断としてグローバル hop ごとに1回**記録（keyword 別に分けない）。逐次版は keyword ごとに別 hop で split し得るため**件数は逐次合算と一致しない**＝これは §8.4 全件性の対象外（走査構造に依存する内部診断）と明記。
- **§8.4 全件性カテゴリとの整合**: `symbol_rejected(capped)`/`decode_replaced` 等の §8.4「全件報告」カテゴリは上記 keyword 別マージで逐次版と件数・順序を一致させる。`automaton_split` のみ走査構造依存で非対象。
...
| automaton＋tree-sitter | **union ヒット和集合** × グローバルhop | lazy parse（実装済）＋共有。注: union ヒット和集合は個別 keyword 版の和集合の上位＝parse ファイル数が個別版より増え得る（出力不変・性能のみ）。§9 実測。 |
| 診断メモリ（rev.4） | keyword 別 Diagnostics 35本＋35 ChaseState 同時常駐 | `decode_replaced` 等は全件追記で青天井。§9(II) で RAM 実測対象に明示。必要なら hop 毎フラッシュ（keyword タグ付き→stable sort マージ）で常駐回避 |

**候補上限の定量（rev.4・必達トリガ）**: 初回 chardet が支配項のとき、**1時間に収まる初回hop候補（非UTF-8分）の上限 ≈ `3600s × 0.1MB/s × nproc`**。例: nproc=16 なら **約 5.6GB（全60GBの約10%）**。候補非UTF-8量がこれを超えたら option A は初回hopだけで予算超過＝**即 option B(cchardet) トリガ**。memo は初回 chardet を救わない（§4.2 コスト床）。

**rev.3〜4 での初回hopの是正と残リスク**: rev.2 §4.4 のままだと初回hop全60GBデコード（＝全chardet）で option A 構造破綻だった。rev.3 §4.4（非ASCII透過のみ walk 段保留・enc-memo 非依存）で初回hopから prefilter 有効・候補有界化。**ただし候補有界化は「rg `-F` 部分文字列 vs automaton 語境界」のギャップで*統計的に*破れ得る**（ありふれ識別子 seed→候補爆発、§4.4 候補爆発リスク）。よって option A の1時間成否は **§9 のプローブ＋実測で判定**し、上限超なら §10(b)。

**walk コストは独立した要対処項**。ロックステップで prefilter を圧縮しても walk が 25〜50分なら目標を脅かす。**決定木で確定**: §9(I) で walk を実測 →（a）25分未満なら無対処（既定）／（b）超過なら「既知ソース拡張子で `_is_binary` 8KB read を短絡」を採用（`walk_skipped_binary` 診断が変わるため §8.4 明記）。並列化は `seen_real` の first-wins 順序依存で `symlink_dedup` 代表が変わり得るため**非採用**（決定性リスク）。
...
**(I) 実コーパス軽量プローブ（実装前・手戻り防止の要）**: 紙上式だけでは候補バイト B を見積もれない（`-F` 偽陽性・stoplist 刈り込みは実 seed 依存）。**大規模実装の前に、実コーパス（または代表サブツリー数GB）で次を測る**:
1. 実 `.grep` 35個の **初回 union 記号**（direct ヒット→chase 抽出→stoplist/min_specificity 適用後）を算出。
2. その union で `rg -l -F`（＋検証できれば `-w` も）を打ち、**候補ファイル総バイトと非UTF-8割合**を測る＝B の実値。
3. B を上限式 `3600 × 0.1MB/s × nproc` と比較。**B が上限以下なら option A 続行、超なら option B(cchardet) を主軸に切替**。
4. walk を実測（ファイル数 × (stat+read+realpath)）。25分超なら §8 決定木(b)。
- これにより「実装してから1時間未達が判明」を回避し、option A/B を**実装前に確定**できる。

**(I-紙上) 上限式**: 初回 chardet 時間 ≈ `B / (0.1MB/s × nproc)`、walk ≈ ファイル数 × (stat+read+realpath)。
...
| importlib 実体パス前提／sha256 頻度／available 副作用 | Medium | §4.5 ディレクトリインストール前提（非 FS Traversable は None 縮退）・sha256 run 1回・available 副作用フリー |
...
| 指摘 | 重大度 | 反映 |
|---|---|---|
| rg `-F` 部分文字列 vs automaton 語境界のギャップで候補が統計的に爆発（ありふれ識別子 seed→ほぼ全60GB候補→初回 chardet 再来） | Critical(性能) | §4.4 候補爆発リスク明記（stoplist 緩和・`-w` 要検証）／§9 実コーパス軽量プローブで実装前に B 実測し A/B 確定 |
| EBCDIC 等 BOM/NUL 無し非ASCII透過 enc が walk 段検出を逃れ C1 残存 | High | §4.4 v1 スコープ前提で想定外除外（一次防御）＋walk 段判定（二次）・§8.4 拡張 |
| 1時間に収まる候補上限が未定量 | High | §8 上限式 `3600×0.1×nproc`≈10% を必達トリガ化 |
| walk 方式が二択のまま | High | §8 決定木で確定（既定無対処→25分超で拡張子短絡。並列化は dedup 順序リスクで非採用） |
| §6.2 merge のソース横断連結順が未確定 | High | §6.2 「共有walk→direct(kw sorted)→indirect(kw sorted)」で一意化・`automaton_split` は共有hop1回 |
| union グローバル予算の二択曖昧／automaton_split 所属衝突 | High | §4.1 予算式を集合演算で一意確定・§6.2 と整合 |
| `_is_binary` 8→64KiB が walk_skipped_binary 診断を変える | High | §4.4/§8.4 明記 |
| 35 Diagnostics＋35 state のメモリ／union tree-sitter parse 増 | Medium | §8 表に明記・§9 RAM 実測対象 |
