...
- `local_variable_declaration` / `field_declaration` / `resource`（try-with-resources のリソース宣言・レビュー I-3 反映で追加・旧 regex は `r` を拾うため非追加は退行）:
  ...
  - 名前は子の `variable_declarator` の `name` フィールド（`identifier`）から（複数宣言子は全件・出現順）。`resource` は `name` フィールド直下の `identifier`（plan で実 grammar 確認）。
...
7. master spec **§390 本文を「完了」へ更新し対象に proc を明記**、§14 の後続候補リストから本項を完了側へ移す（master が source of truth・MEMORY 方針）。
