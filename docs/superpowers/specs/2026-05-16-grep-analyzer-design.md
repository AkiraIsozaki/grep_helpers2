...
- **v2 追加済（正規表現トラック・フェーズ5）**: **PL/SQL（新 language を作らず `sql`＝Oracle 方言へ統合）/ Perl / Groovy**（行単位正規表現・confidence=medium・新依存ゼロ）。詳細は差分設計書 `docs/superpowers/specs/2026-05-22-v2-regex-languages-design.md`。シェバン検出は perl/groovy へ一般化（python は track A 未着手で `unsupported_shebang` 据置）。
...
- 残り言語（Kotlin / TS・JS / Python / C#・VB.NET）を §5.2 プラグイン経由で追加（TS・JS / Python は tree-sitter トラック=track A、JSP / Angular は埋め込みトラック=track C として継続。Kotlin・C#/VB.NET は現ユーザースコープ外）。
...
- **フェーズ5（v2 正規表現トラック言語追加・完了済）**: 差分設計書 `docs/superpowers/specs/2026-05-22-v2-regex-languages-design.md` を正本に、PL/SQL（`sql` 統合）/ Perl / Groovy を行単位正規表現で追加（フェーズ1〜4 完了の決定的基盤＋出力スキーマ上に乗せる）。着手前提＝フェーズ1〜4 完了済。完了基準＝**Inv-B（v1 既存 golden を byte 不変＋既存 unit/integration green）** ＋ §11 必須回帰（言語×ref_kind golden・偽陽性抑止の予約語ストップリスト・PL/SQL 複数代入温存・perl/groovy var→chain）。新依存ゼロ（tree-sitter 不使用）。実装は subagent-driven-development で TDD・タスク別二段レビューにて完了。track A（TS/JS・Python＝tree-sitter）/ track C（JSP・Angular＝埋め込み）は後続フェーズ。
