...
- 改訂:
  ...
  - **v9（v8 批判的AIレビュー収束反映）: 3観点並列レビューの Critical/Important を反映。golden 比較機構の normative 規定（§11）／`file` 解決規則の一意化と §8.2 用語整合（§8.2/§9）／Pro\*C EXEC区間 snippet 専用規則（§7/§9）／tree-sitter 構文別ノード対応表＋同一行複数 statement・複数行式・parse 失敗フォールバックの一意化（§9）／上限縮約の擬似コード（§9）／sql/shell ヒューリスティックの走査方向・mask 整合・大小無視の確定（§9）／全順序性の論証と残余全列タイブレーク（§9）／サニタイズ対象集合拡張と区切り衝突エスケープ（§9）／diagnostics は relpath 維持の層分離（§10.3）／part 分割は行数基準で不変・`_ITEMS_PER_MB`/perf 再測定（§9/§8.2）／Inv-1 再ベースラインと既存 expected 全件再生成を §11/§15 へ明記。**反復2 追加収束**: `_node_at_line` は最小葉トークンを返す実体に合わせ「段1=最小内包ノード→段2=`.parent` 上昇で粒度表構文へ昇格」を §9 に明記（Crit-2）／Pro\*C EXEC 区間検出を**リテラル空白化コピー上**で行い文字列内 `;` 誤切断を回避（§7・§8.4 境界・Crit-1）／Java `try_with_resources_statement`・C `case_statement` ノード規則の一本化（§9）／`chase.mask_literals` は行単位・heredoc 非対応の実体を明記し過大評価を撤回（§9/§8.4）／golden 比較は manifest encoding で読戻し＋resume 系は `is_complete` も assert（§11）／`{SOURCE_ROOT}` 逆置換は `SR+"/"` 先頭1回のみ（§11）／Inv-1 凍結証跡を「数値」でなく `phase0-gate-result.md` 該当節の行番号参照へ訂正（§15・C-2'）／フェーズ4 着手循環の解消＝golden 固定は writing-plans 成果物（§15・I-3'）／symlink golden は `--follow-symlinks` 前提を明記（§11）**
  - **v10（現行・配備先ピボット: x86_64→aarch64/DGX Spark）: 想定配備先を NVIDIA DGX Spark（GB10 Grace Blackwell＝Arm/aarch64・DGX OS=Ubuntu 24.04 系・system Python 3.12）に確定。§4.2 G1 を aarch64 で再ベースライン（`phase0-gate-result.md` の「aarch64 再ベースライン記録」節が正本）。(1) `pyahocorasick` は旧 pin `==2.1.0` に aarch64 prebuilt wheel が皆無のため §4.2(b)＝版選択として `==2.3.1` へ更新（`automaton.py` 使用 API＝`Automaton/add_word/make_automaton/iter` 不変、全テスト 244 件 byte 同値で互換実証）。(2) wheelhouse を多アーキ同梱化（x86_64＝開発/CI ＋ aarch64＝配備。ネイティブ4件×2アーキ／純Python共通）し `pip install --no-index --require-hashes` が両アーキで成立。`scripts/gen_requirements_lock.py` を同一 `pkg==ver` の複数 `--hash` 集約へ改修、`tests/unit/test_requirements_lock.py` を全 hash 検証へ強化。(3) 既存潜在不具合の是正＝オフライン `pip install -e .` のビルド分離が `setuptools(>=68)` 不在で失敗していたため `setuptools`/`wheel`（純Python・アーキ非依存）を wheelhouse 同梱。(4) `pyproject.toml` の `dependencies` に `pyahocorasick==2.3.1` を明記（従来 import のみで未宣言だった不整合を是正）。**出力スキーマ・決定性・Inv-1 は不変＝再ベースライン不要**。**
- ステータス: ドラフト（v10＝配備先 aarch64 ピボット・§4.2 G1 再ベースライン反映済、v9＝v8 批判的AIレビュー収束反映済・ユーザー再レビュー待ち）。v8/v9 は Phase3 完了後の意図的破壊的スキーマ変更（Inv-1 再ベースライン）。v10 はスキーマ不変（依存供給網のアーキ追従のみ）
...
| 項目 | 内容 |
|---|---|
| 稼働先 | **モダンLinux・オフライン**（v10 確定: NVIDIA DGX Spark＝aarch64 / DGX OS=Ubuntu 24.04 系・glibc≥2.17 / Python 3.12。G1 再ベースライン正本＝`phase0-gate-result.md`） |
...
| **要確定欄（実装前ゲート G1）** | **確定（v10・G1 aarch64 再ベースライン）: CPUアーキ＝aarch64 / libc＝実 glibc 2.39（DGX OS=Ubuntu 24.04 系。wheel 要件は manylinux2014＝glibc≥2.17 を採用し下限可搬性確保） / Python ABIタグ＝cp312**。wheel 入手可否を決める（§4）。〔履歴: v0〜v9 は x86_64/glibc2.36 で G1 PASS。v10 で配備先 DGX Spark 確定により aarch64 再実証〕 |
...
- **v2 追加済（正規表現トラック・フェーズ5）**: **PL/SQL（新 language を作らず `sql`＝Oracle 方言へ統合）/ Perl / Groovy**（行単位正規表現・confidence=medium・新依存ゼロ）。詳細は差分設計書 `docs/superpowers/specs/2026-05-22-v2-regex-languages-design.md`。
- **v2 追加済（tree-sitter トラック A・2026-05-23）**: **TypeScript / TSX / JavaScript / Python**（AST field-directed chaser・classify=high・direct＋indirect 多ホップ）。詳細は差分設計書 `docs/superpowers/specs/2026-05-23-v2-tree-sitter-languages-design.md` と実装計画 `docs/superpowers/plans/2026-05-23-v2-tree-sitter-languages.md`。シェバン検出を python/node へ一般化（`unsupported_shebang` 据置を解消）。
- **v2 追加済（埋め込みトラック C・2026-05-23）**: **JSP / Angular / HTML**（Pro\*C 逆マスク方式・新規 grammar wheel ゼロ）。詳細は差分設計書 `docs/superpowers/specs/2026-05-23-v2-track-c-embedded-design.md` と実装計画 `docs/superpowers/plans/2026-05-23-v2-track-c-embedded.md`。JSP は行ベース java_chaser（逆マスク後 java grammar）、Angular は AST typescript_chaser（逆マスク後 typescript grammar）、HTML はパススルー（その他・chase なし）。v2 ロードマップ（B→A→C）完遂。
...
- **v10 確定ピン**: `pyahocorasick==2.3.1`（旧 `==2.1.0` は aarch64 prebuilt wheel が皆無のため §4.2(b)＝版選択で更新。tree-sitter 系3点・`chardet` は版不変、aarch64 prebuilt 入手済）。**wheelhouse は多アーキ同梱**＝開発/CI=x86_64 と配備=aarch64 の双方で `pip install --no-index --find-links wheelhouse --require-hashes -r requirements.lock` が成立する（ネイティブ4点は x86_64/aarch64 の2 wheel、純Python は共通1 wheel。`requirements.lock` は同一 `pkg==ver` を複数 `--hash` に集約）。**ビルドバックエンド `setuptools(>=68)`/`wheel` も wheelhouse 同梱**（純Python・アーキ非依存）＝オフライン `pip install -e .` のビルド分離をネット不要で成立させる（旧来欠落していた供給網の穴を v10 で是正）。
...
- **非シェルのシェバン（v1 旧記述）**: track A 完了により python/node シェバンは正常ディスパッチ済み（`unsupported_shebang` 据置を解消）。未対応シェバンは引き続き diagnostics `unsupported_shebang` へ記録（§5.1）。
- **track A（TS/TSX/JS/Python）の既知限界**: (1) 動的プロパティアクセス（`obj[key]`）は `attribute`/`subscript` 左辺を抽出しない仕様（spec §6.5 field-directed）。(2) getter/setter の terminal 非拡張は `confidence=low` でのみ出力（型解決不要の静的規則）。(3) 単一行クラス本体内の `method_definition` 抽出は `binding_at_line`（決定的最小スパン探索・ts_classifier 集約・3言語共用）で対応済み。(4) JSX/TSX の props spread・template literal 内の式は追跡未対応。
- **track C（JSP/Angular/HTML）の既知限界**: (1) **Angular inline template**（`.component.ts` 内 `template:` リテラル）は **§389 完了（2026-05-23）により対応済み**（実効言語 `angular_inline`・TSV `language` 列は `typescript`・差分設計書 `docs/superpowers/specs/2026-05-23-angular-inline-template-design.md` §6 に既知限界を列挙）。**inline template 対応の既知限界**（差分設計書 §6 から継承）: `template:` がコメント/文字列内に出る稀ケースで誤検出（best-effort）・テンプレ内容にネスト/エスケープ template literal があると非貪欲マッチ早期終端（稀）・`.tsx` 非対象（Angular 非使用）・`@Component` デコレータ非限定（`template:` パターンのみ）・**TSV `language` 列は inline template ヒットでも `typescript`**（外部テンプレ `.html`（language=`angular`）と非対称）・テンプレ内 JS `${}` 式は chase 空（直接ヒットは出るが indirect chase 未対応）。(2) **JSP 標準アクション**（`<jsp:useBean>`/`<jsp:setProperty>` 等）は逆マスクで空白化され非追跡。(3) **JSTL カスタムタグ束縛**（`<c:set var="x">`）・カスタム EL 関数は bundle chase せず（EL `${…}` 直接ヒットは捕捉）。(4) **jsp 行ベース chaser の属性偽抽出**（`<c:set var="x" value="${T}"/>` の属性名 `var`/`value` を変数として過抽出しうる・best-effort）。(5) **JSP 文字列リテラル内の `%>`**（非貪欲マッチ早期終端・稀ケース・best-effort）。(6) **多行 scriptlet**（§390 の java/c chaser AST 化（完了・2026-05-24）により行跨ぎ束縛を追跡可能になり解消。`bindings_at_line` が行交差する外側宣言ノードを捕捉）・**多行 Angular 補間**（`{{ a\n.b }}` は部分抽出）。(7) **Angular `#ref`・パイプ名・`*ngFor` 別名**（`as`/`index as i`/`trackBy:` は非追跡）。(8) **HTML コメント `<!-- … -->`** は jsp/angular とも空白化（コメント内 `{{}}` / `<% %>` の誤抽出回避）。(9) **`.html` ヒューリスティック誤判定（両方向）**（Angular 固有マーカ無しのテンプレ→html 扱い・静的 HTML に Angular マーカ混在→angular 扱い）。詳細は差分設計書 `docs/superpowers/specs/2026-05-23-v2-track-c-embedded-design.md` §6。
- **生成コード**（既定除外。除外した旨を記録）。
...
| `confidence` | `high`/`medium`/`low`（ヒット単位）。**`low` の意味（2026-05-24 一本化）＝ 当該行が keyword の真のコード使用である確信度が低い ＝ ① 型解決を要する間接 getter/setter（§8.3）＋ ② 非コード行＝`コメント` カテゴリ（§7）** |

**サニタイズ規約（v9・全セル共通・決定性必須）**: 各セル文字列は連結・出力前に次を順に適用する。① `tsv._sanitize` を拡張し **`\t` `\r` `\n` `\v`(U+000B) `\f`(U+000C) `U+0085`(NEL) `U+2028`(LS) `U+2029`(PS) を半角空白1個へ置換**（Excel・TSVビューアでの行/改ページ分割を防ぎ「1セル」目的を担保。`split("\n")`/`data_sha256` の決定性コアは `\n`→空白で従来どおり不変）。② snippet 連結時、**ソース物理行テキスト中に区切り列 ` \n `（U+0020 U+005C U+006E U+0020）と同一の3文字並びが出現した場合**、その出現の `\`(U+005C) を `\\`(U+005C U+005C) へ決定的にエスケープしてから ` \n ` で連結する（区切りと本文の曖昧化を防ぐ・逆変換は将来 §14 のみ必要なので v1 はエスケープのみ規定）。③ ①②は `tsv.py:_sanitize`／`output_writer._data_line` の改修を伴う＝v8/v9 は当該コアの**対象集合拡張という改修**を含む（「決定性アルゴリズムは不変・対象文字集合のみ拡張」と正確に理解する）。**（2026-05-24 改訂）** ①の空白化対象を「全制御文字（C0 U+0000–U+001F／DEL U+007F／C1 U+0080–U+009F）＋ U+2028 ＋ U+2029」へ拡張する（旧列挙 `\t \r \n \v \f U+0085 U+2028 U+2029` を包含・決定性アルゴリズム不変＝対象集合のみ拡張。ソース由来の生制御バイトが出力 TSV セルへ混入するのを防ぎ「1セルを保つ」intent を完全化）。回帰は §11（`source_ctrl_chars`）。
...
- **java / c（tree-sitter・分類と同一 AST。Pro\*C は §7 の専用規則を先に適用し、EXEC 区間外のみ本規則）**: ヒット行に対応する選択ノードを**2段で**一意に決める（v9 訂正: 実装 `ts_classifier._node_at_line` は対象行を**内包**するノードを子へ降りながら `(行スパン, start_byte, end_byte, node.type)` 最小で選ぶため、戻り値は statement ではなく**最小の葉トークン**＝`identifier` 等。よって「同一タイブレークで最左 statement が選ばれる」とは書けない。`classify_ts` が `.parent` を遡って文レベルへ昇る挙動と一貫させる）:
  ...
  - **粒度（上位優先・構文別の具体ノードへ落とす。括弧範囲＝対応する `(`/`)` トークンの物理行スパン）**:
    ...
    | if / while | `if_statement`/`while_statement` の `condition`(`parenthesized_expression`) | 同左 `parenthesized_expression` | `(`〜対応する `)` の**物理行スパン**（行単位切り出しのため `)` と同一物理行に続く `{` 等は含む。**本体ブロックの後続行は非包含**＝条件が複数行ならその行範囲のみ） |
  ...
  - **フォールバック順（決定的・§10.3 部分パース継続と接続）**: (1) 上記でノードが取れる → そのスパン。**ERROR/MISSING 判定は選択ノードの部分木のみ**（祖先方向に遡上しない＝ファイル内の無関係なエラーで健全行を捨てない・§10.3「部分木で継続」と一貫。判定は `node.is_missing or node.type=="ERROR" or node.has_error`＝`has_error` は子孫包含）。(2) java/c で (1) が `None`（parse 失敗・選択ノードが ERROR/MISSING を含む・該当構文無し）→ **`hit` 1物理行のみ**（java/c に sql/shell ヒューリスティックは適用しない＝C へ shell 規則を当てる非整合を排除）。**Pro\*C は §7 の専用順**（EXEC 区間＝原ソース span／区間外＝`mask_exec_sql` 後の C ノード規則／それも不能なら `hit` 1物理行）。sql/shell は本「snippet 切り出し規則」の sql/shell ヒューリスティックが (1) 相当（AST 非使用）であり、フォールバックは `hit` 1物理行。
...
- **上限と切り詰め（決定的・擬似コード）**: 行数上限 **既定 12 行**、文字数上限 **既定 800 文字**（最終 ` \n ` 連結後・サニタイズ後・省略マーカ込みの文字列長で測定）。上限は v1 spec 固定（CLI 可変は §14）。
  ...
  - マーカ厳密書式（v9 確定・上/下を区別＝L276-277 擬似コードと一致）: 上端省略は `…(+<K>上行省略)`、下端省略は `…(+<K>下行省略)`（`…`＝U+2026、`<K>`＝10進・ゼロ埋め無し、`K`＝当該側の省略物理行数）。マーカは ` \n ` 連結に含めず**先頭/末尾セルに直接前置/後置**し、**行数 LINE_MAX にはカウントしない／文字数 CHAR_MAX 測定には含める**。ヒット行単独が CHAR_MAX 超過のときの `truncate_codepoints` は Unicode コードポイント先頭から `CHAR_MAX-1` 字を採り末尾に `…`(U+2026) を付す決定的切詰（バイト単位ではない・両端行マーカとは別概念）。**本書内の `…(+K行省略)` という簡略表記（§8.4・§11）は本厳密書式の総称であり、実装・golden は上記 上/下 区別形が正本**。
...
- **v2 で対応済（差分設計書 2026-05-22・フェーズ5）**: PL/SQL（`sql` 統合・正規表現）/ Perl / Groovy。
- **v2 で対応済（tree-sitter トラック A・2026-05-23）**: TypeScript / TSX / JavaScript / Python（§2.3・差分設計書 `docs/superpowers/specs/2026-05-23-v2-tree-sitter-languages-design.md`）。
- **v2 で対応済（埋め込みトラック C・2026-05-23）**: JSP / Angular / HTML（差分設計書 `docs/superpowers/specs/2026-05-23-v2-track-c-embedded-design.md`）。**v2 ロードマップ（B→A→C）完遂**。Kotlin・C#/VB.NET は現ユーザースコープ外。
- **Angular inline template の式抽出（完了・2026-05-23）**: `.component.ts` 内 `template:` テンプレートリテラルの式（`{{}}`/`[x]=`/`*ngFor` 等）を実効言語ルーティング（`angular_inline`）で抽出。dispatch 言語は `typescript` のまま・TSV `language` 列も `typescript`（§2.2 規約）。差分設計書 `docs/superpowers/specs/2026-05-23-angular-inline-template-design.md` と実装計画 `docs/superpowers/plans/2026-05-23-angular-inline-template.md` を参照。完了基準＝**Inv-D（既存全 golden byte 不変）** ＋ `angular_inline_chain` golden（テンプレ内 `*ngFor` row 多ホップ・language=typescript）＋ 全テスト 381 passed, 5 skipped。§390 完了（2026-05-24）により v2 後続候補は完遂。
- **既存 java/c/jsp/proc chaser の AST 化統一（完了・2026-05-24）**: java/c/jsp/proc（Pro*C）chaser を行単位正規表現ベースから tree-sitter AST（field-directed・multi-node）へ統一。`bindings_at_line`（複数ノード版コレクタ・行交差全件・決定的順）を `ts_classifier.py` に新設し、`java_chaser.extract_tree` と `c_chaser.extract_tree` を実装（jsp は java 流用・proc は c 流用＋mask_exec_sql で EXEC SQL 区間を空白化）。classify/chase 基盤を AST に一本化し `_AST_CHASERS` へ移動。既存 golden byte 不変（Inv-A/Inv-C）＋新規 golden 3件（java_multiline_chase・proc_exec_sql_chase・c_compound_assign）で AST 能力を固定。差分設計書 `docs/superpowers/specs/2026-05-23-chaser-ast-unification-design.md` と実装計画 `docs/superpowers/plans/2026-05-23-chaser-ast-unification.md` を参照。完了基準＝multi-node `bindings_at_line`・name 行ゲート（method 系のみ）・既存 golden byte 不変・新規 golden 3件・395 passed, 5 skipped。
- Shell を tree-sitter-bash 化（任意）。
- **tree-sitter Oracle/PL-SQL grammar 採用**（SQL 分類 medium→high・§8.1 複数行 PL/SQL 抽出精度向上）。**前提条件: §4.2 G1 相当の検証で、py-tree-sitter 0.23.x ABI と整合する Oracle/PL-SQL grammar の cp312・対象 manylinux prebuilt wheel がオフライン取得可能であること**。不可なら sdist ビルド可否を §3/§4.2 と合わせ意思決定（§12 の再評価を参照）。
...
- **フェーズ5（v2 正規表現トラック言語追加・完了済）**: 差分設計書 `docs/superpowers/specs/2026-05-22-v2-regex-languages-design.md` を正本に、PL/SQL（`sql` 統合）/ Perl / Groovy を行単位正規表現で追加（フェーズ1〜4 完了の決定的基盤＋出力スキーマ上に乗せる）。着手前提＝フェーズ1〜4 完了済。完了基準＝**Inv-B（v1 既存 golden を byte 不変＋既存 unit/integration green）** ＋ §11 必須回帰（言語×ref_kind golden・偽陽性抑止の予約語ストップリスト・PL/SQL 複数代入温存・perl/groovy var→chain）。新依存ゼロ（tree-sitter 不使用）。実装は subagent-driven-development で TDD・タスク別二段レビューにて完了。
- **フェーズ6（v2 tree-sitter トラック A・完了済・2026-05-23）**: 差分設計書 `docs/superpowers/specs/2026-05-23-v2-tree-sitter-languages-design.md` を正本に、TypeScript / TSX / JavaScript / Python を tree-sitter AST field-directed chaser で追加。tree-sitter を 0.21.3→0.23.x へ移行（java 0.23.5 / c 0.23.4 / python 0.23.6 / javascript 0.23.1 / typescript 0.23.2）。完了基準＝**Inv-A（既存 java/c/proc/sql/shell/perl/groovy golden byte 不変）** ＋ 4言語 direct＋indirect 多ホップ golden（py_chain・js_kinds・ts_enum_readonly・tsx_chain）＋ 全テスト 331 passed, 5 skipped（track A 完了時点）。
- **フェーズ7（v2 埋め込みトラック C・完了済・2026-05-23）**: 差分設計書 `docs/superpowers/specs/2026-05-23-v2-track-c-embedded-design.md` と実装計画 `docs/superpowers/plans/2026-05-23-v2-track-c-embedded.md` を正本に、JSP / Angular / HTML を Pro\*C 逆マスク方式で追加。**新規 grammar wheel ゼロ**（jsp→tree-sitter-java・angular→tree-sitter-typescript を host grammar として流用・G1 自動充足）。Phase C1（JSP＋HTML パススルー）→Phase C2（Angular）の2フェーズで実装。完了基準＝**Inv-C（既存全言語 golden byte 不変）** ＋ jsp/html/angular golden（jsp_chain・html_passthrough・angular_chain）＋ 全テスト 366 passed, 5 skipped。**v2 ロードマップ（B→A→C）完遂**。既知限界は §8.4 track C の節を参照。
