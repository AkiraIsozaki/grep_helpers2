...
> **track A G1 再ベースライン（2026-05-23・tree-sitter 0.21→0.23.x・4言語 grammar 追加）**: track A（TS/TSX/JS/Python）実装に伴い tree-sitter を `0.23.2`、grammar を java `0.23.5` / c `0.23.4` / python `0.23.6` / javascript `0.23.1` / typescript `0.23.2` へ更新。x86_64/aarch64 両アーキ wheel を wheelhouse 同梱済み（計12 grammar wheel＋既存）。詳細は差分設計書 §2 と `phase0-gate-result.md`（aarch64 再ベースライン記録節が正本）。`requirements.lock` のテスト件数ベースラインは **366 passed, 5 skipped**（track C 完了時点。track A 完了時点は 331 passed, 5 skipped）。
...
- **track C（JSP/Angular/HTML）の既知限界**: (1) **Angular inline template**（`.component.ts` 内 `template:` リテラル）は抽出しない（`.ts` は track A typescript 処理・テンプレ文字列内の束縛解析は後続候補）。(2) **JSP 標準アクション**（`<jsp:useBean>`/`<jsp:setProperty>` 等）は逆マスクで空白化され非追跡。(3) **JSTL カスタムタグ束縛**（`<c:set var="x">`）・カスタム EL 関数は bundle chase せず（EL `${…}` 直接ヒットは捕捉）。(4) **jsp 行ベース chaser の属性偽抽出**（`<c:set var="x" value="${T}"/>` の属性名 `var`/`value` を変数として過抽出しうる・best-effort）。(5) **JSP 文字列リテラル内の `%>`**（非貪欲マッチ早期終端・稀ケース・best-effort）。(6) **多行 scriptlet**（java_chaser 単行制約・行跨ぎ束縛非追跡）・**多行 Angular 補間**（`{{ a\n.b }}` は部分抽出）。(7) **Angular `#ref`・パイプ名・`*ngFor` 別名**（`as`/`index as i`/`trackBy:` は非追跡）。(8) **HTML コメント `<!-- … -->`** は jsp/angular とも空白化（コメント内 `{{}}` / `<% %>` の誤抽出回避）。(9) **`.html` ヒューリスティック誤判定（両方向）**（Angular 固有マーカ無しのテンプレ→html 扱い・静的 HTML に Angular マーカ混在→angular 扱い）。詳細は差分設計書 `docs/superpowers/specs/2026-05-23-v2-track-c-embedded-design.md` §6。
- **生成コード**（既定除外。除外した旨を記録）。
...
- **Angular inline template の式抽出（後続候補）**: `.component.ts` 内 `template:` テンプレートリテラルの式（`{{}}`/`[x]=`/`*ngFor` 等）を track C 相当の逆マスクで抽出する（現在は `.ts` 本体のみ track A typescript 処理）。track C の `extract_angular_ts` を再利用し template リテラル検出+内容抽出を追加すれば実現可能。
