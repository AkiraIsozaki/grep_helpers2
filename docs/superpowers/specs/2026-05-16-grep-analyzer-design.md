...
> **track A G1 再ベースライン（2026-05-23・tree-sitter 0.21→0.23.x・4言語 grammar 追加）**: track A（TS/TSX/JS/Python）実装に伴い tree-sitter を `0.23.2`、grammar を java `0.23.5` / c `0.23.4` / python `0.23.6` / javascript `0.23.1` / typescript `0.23.2` へ更新。x86_64/aarch64 両アーキ wheel を wheelhouse 同梱済み（計12 grammar wheel＋既存）。詳細は差分設計書 §2 と `phase0-gate-result.md`（aarch64 再ベースライン記録節が正本）。`requirements.lock` のテスト件数ベースラインは **329 passed, 5 skipped**（track A 完了時点）。
...
- **track A（TS/TSX/JS/Python）の既知限界**: (1) 動的プロパティアクセス（`obj[key]`）は `attribute`/`subscript` 左辺を抽出しない仕様（spec §6.5 field-directed）。(2) getter/setter の terminal 非拡張は `confidence=low` でのみ出力（型解決不要の静的規則）。(3) 単一行クラス本体内の `method_definition` 抽出は `_binding_at_line`（BFS 最小スパン探索）で対応済み。(4) JSX/TSX の props spread・template literal 内の式は追跡未対応。
- **生成コード**（既定除外。除外した旨を記録）。
...
- **既存 java/c chaser の AST 化統一（後続候補）**: 現在の java/c chaser は行単位正規表現ベース。track A の `_binding_at_line` BFS 探索・field-directed 抽出方式を java/c に適用すれば classify=high と統一できる（現状は AST classify を使いながら chaser は非 AST のハイブリッド）。後続フェーズの候補として記録（Inv-A 既存 golden byte 不変を維持したまま移行要件あり）。
...
- **フェーズ6（v2 tree-sitter トラック A・完了済・2026-05-23）**: 差分設計書 `docs/superpowers/specs/2026-05-23-v2-tree-sitter-languages-design.md` を正本に、TypeScript / TSX / JavaScript / Python を tree-sitter AST field-directed chaser で追加。tree-sitter を 0.21.3→0.23.x へ移行（java 0.23.5 / c 0.23.4 / python 0.23.6 / javascript 0.23.1 / typescript 0.23.2）。完了基準＝**Inv-A（既存 java/c/proc/sql/shell/perl/groovy golden byte 不変）** ＋ 4言語 direct＋indirect 多ホップ golden（py_chain・js_kinds・ts_enum_readonly・tsx_chain）＋ 全テスト 329 passed, 5 skipped。track C（JSP / Angular＝埋め込みトラック）は後続。
