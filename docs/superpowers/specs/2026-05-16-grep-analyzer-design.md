...
> **track A G1 再ベースライン（2026-05-23・tree-sitter 0.21→0.23.x・4言語 grammar 追加）**: track A（TS/TSX/JS/Python）実装に伴い tree-sitter を `0.23.2`、grammar を java `0.23.5` / c `0.23.4` / python `0.23.6` / javascript `0.23.1` / typescript `0.23.2` へ更新。x86_64/aarch64 両アーキ wheel を wheelhouse 同梱済み（計12 grammar wheel＋既存）。詳細は差分設計書 §2 と `phase0-gate-result.md`（aarch64 再ベースライン記録節が正本）。`requirements.lock` のテスト件数ベースラインは **381 passed, 5 skipped**（§389 Angular inline template 完了時点。track C 完了時点は 366 passed, 5 skipped・track A 完了時点は 331 passed, 5 skipped）。
...
- **Angular inline template の式抽出（完了・2026-05-23）**: `.component.ts` 内 `template:` テンプレートリテラルの式（`{{}}`/`[x]=`/`*ngFor` 等）を実効言語ルーティング（`angular_inline`）で抽出。dispatch 言語は `typescript` のまま・TSV `language` 列も `typescript`（§2.2 規約）。差分設計書 `docs/superpowers/specs/2026-05-23-angular-inline-template-design.md` と実装計画 `docs/superpowers/plans/2026-05-23-angular-inline-template.md` を参照。完了基準＝**Inv-D（既存全 golden byte 不変）** ＋ `angular_inline_chain` golden（テンプレ内 `*ngFor` row 多ホップ・language=typescript）＋ 全テスト 381 passed, 5 skipped。残る後続候補は §390（chaser AST 化統一）のみ。
- **既存 java/c/jsp chaser の AST 化統一（後続候補）**: 現在の java/c chaser（jsp は java_chaser 流用）は行単位正規表現ベース。track A の `binding_at_line` 決定的最小スパン探索・field-directed 抽出方式を java/c/jsp に適用すれば classify=high と統一できる（現状は AST classify を使いながら chaser は非 AST のハイブリッド）。後続フェーズの候補として記録（Inv-A/Inv-C 既存 golden byte 不変を維持したまま移行要件あり）。
