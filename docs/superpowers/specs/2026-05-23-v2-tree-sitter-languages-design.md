...
```
新契約:
  ingest_one(state, parent, language, chase_symbols: ChaseSymbols, kinds: dict[str,str], hop, is_seed=False)
    └ 抽出専用の dialect/line のみ引数から除く（抽出は呼出側責務）。**language は保持する**
      （partition→admit が LANG_KEYWORDS.get(language) で keyword 篩に使う＝§6.7 stoplist と
       既存 java/c/sql/... の keyword 除外の両方に必須。落とすと keyword が chase 入りし golden が変わる）。

  kinds_of(chase_symbols: ChaseSymbols) -> dict[str,str]   # ← line 引数から ChaseSymbols 引数へ変更（言語非依存）
    └ 既存ループ順 (setter, getter, var, constant) の last-write-wins を維持（constant を最後に書く＝優先）。
      ループ順を変えない（並べ替えると優先が反転する）。
```
...
1. **seed**（`fixedpoint/_seed.py`）: seed ファイルを main で読む既存経路を使い、AST 言語は `cs = extract_chase_symbols_tree(lang, text, s.lineno)`、行言語は `cs = extract_chase_symbols(lang, dialect, seed_line)`。**ファイル欠落/未読（`sp.is_file()` False）時は `cs = ChaseSymbols()`（空）**＝現 `seed_line=""` 相当。`kinds = kinds_of(cs)`、`ingest_one(occ, lang, cs, kinds, hop=1, is_seed=True)`（`is_seed` 自己包含は投入側で従来どおり）。
2. **scan worker**（`fixedpoint/_scan.py:_scan_file`）: AST 言語ファイルを **1 回 parse**（`root = parse_tree(lang, text)`・既に text 保持）し、found 各 occurrence について `cs = extract_chase_symbols_from_root(lang, root, lineno)` を**同一 root を共有して worker で計算**（file 単位1パース）し found タプルへ同梱。→ **「親に bytes 非常駐」の streaming 設計を維持**・並列のまま・file 単位 1 パース。tuple へ載るのは `ChaseSymbols`（frozen dataclass of tuples＝pickle 安全）と primitives のみ＝tree-sitter オブジェクトは越境させない（`spawn` 安全）。**前提**: `_AST_CHASERS` の chaser モジュールは import 時に parse しない（import-clean・Parser は遅延生成）こと＝worker の `spawn` 再 import と起動コスト境界を保証。
3. **absorb_results**（`fixedpoint/_ingest.py`）: found を**新タプル arity で unpack** し、AST 言語は同梱 `cs` を使用、行言語は `cs = extract_chase_symbols(lang,dialect,line)`（`line` から計算）。`kinds = kinds_of(cs)`、`ingest_one(child, language, cs, kinds, hop+1)`。
...
- **field-directed 抽出（レビュー Critical・必須）**: 束縛ノードの**部分木を無差別 preorder 走査してはならない**（RHS の値・型識別子・プロパティキーを誤抽出する）。**`name:`/`left:` 等の束縛名フィールドのサブツリーのみ**を読み、`right:`/`value:`/`type:` は読まない。抽出順は当該フィールド部分木のソース順（決定的・**set による重複除去は禁止＝順序保持の tuple**）。
  - 例外（アクセサ判定のため値フィールド以外を読む）: `decorated_definition` の先頭 decorator、`method_definition`/`public_field_definition` の匿名 `get`/`set`/`readonly` トークン、`lexical_declaration` の `const`/`let` トークンは読む。これらは get/set/const/readonly 判定のためで、`right:`/`value:`/`type:` の値式は依然読まない。
...
| 言語 | 束縛ノード | 読むフィールド | 規則 |
|---|---|---|---|
| **Python** | `assignment` / `augmented_assignment` | `left:` のみ | `identifier`→§6.4。`pattern_list`/`tuple_pattern`/`list_pattern`→各 `identifier`=var（**ネスト pattern は再帰**＝`a,(b,c)=…` の b,c も）、`list_splat_pattern`(`*r`)→内 identifier=var。`attribute`(`self.x`)/`subscript`(`d[k]`)→**抽出せず**（束縛でなく変更・§8）。chained `a=b=1` は `right:` が `assignment`/`pattern_list` のとき辿り `left:` を追加 |
...
class ASTChaser(Protocol):
    def extract_tree(self, language: str, root, lineno: int) -> ChaseSymbols: ...
    # root = 既に parse 済みの tree-sitter root_node（worker の file 単位1パースを共有）。
    # language は ts/tsx の grammar 変種・束縛規則選択に使う（climb は node_at_line 共通）。
```
- AST 言語の chaser は `extract_tree` のみ公開（`kinds` は返さない＝§6.1 の `kinds_of(cs)` で導出・所有権を一本化）。行版 `extract`/`mask` は持たない。
- **別レジストリ `_AST_CHASERS: dict[str, ASTChaser]`** に登録（typescript/tsx/javascript/python。tsx は typescript_chaser を共用＝grammar 変種は `language` 引数で選ぶ）。行版 `_CHASERS` には登録しない。
- `chase.py` に2ラッパを追加（`_AST_CHASERS` 参照）: `extract_chase_symbols_from_root(language, root, lineno)`（worker 用＝parse 済 root を受ける）と、その薄包み `extract_chase_symbols_tree(language, text, lineno)`（`parse_tree(language, text)` 後に from_root を呼ぶ＝seed/absorb 用）。行言語は既存 `_CHASERS` 経路。
...
- Python 注釈のみ `x: int`（値なし）は `assignment`（`type:` 有・`right:` 無）として `left:` から var 抽出される（型 `int` は `type:` で非抽出・許容）。
...
- **テスト契約デルタ**（レビュー I5・B に倣い明示）: 現状 `ingest_one`/`kinds_of`/`absorb_results`/`scan_hop`/`_scan_file` を**直接呼ぶ既存テストは無い**（文字列言及のみ）ため既存改修は最小。`found` 4-tuple の unpack arity と新シグネチャ（`ingest_one` に `language` 保持・`kinds_of(cs)`）を**新規**表明テストで固定し silent break を検出する。
...
| 7 | `chase.py` | `_AST_CHASERS` 参照の `extract_chase_symbols_tree(language, text, lineno)` 追加。行言語は既存経路、`extract_var_symbols` は AST 非拡張 |
| 8 | `fixedpoint/_scan.py` / `_ingest.py` / `_seed.py` | `kinds_of` を **`(chase_symbols)→dict` へシグネチャ変更**（定義は `_scan.py:47`・import 元 `_ingest.py:15`・§6.1）。抽出/投入分離・新 `ingest_one(state, parent, language, chase_symbols, kinds, hop, is_seed)` 契約・全3呼出点改修（§6.2）・方式A worker AST 抽出・`found` タプル拡張・absorb unpack arity |
