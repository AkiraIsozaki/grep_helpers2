...
- AST 抽出のためマスク不要。AST 言語の chaser は**別レジストリ `_AST_CHASERS`（`extract_tree(text, lineno)→(ChaseSymbols, kinds)` を公開）に登録し、行版 `_CHASERS` には登録しない**（行版 `extract`/`mask` を実装しない）。`mask_literals` は `_CHASERS.get` が None を返し原行をそのまま返す＝heuristic snippet 専用経路で安全（AST 言語は §7 の AST スパンを使用するため呼ばれない）。
...
| 5 | `classifiers/{python,javascript,typescript}_chaser.py`（新規） | AST 抽出 `extract_tree(text, lineno)→(ChaseSymbols, kinds)` 実装（tsx は typescript_chaser 共用）。**別レジストリ `_AST_CHASERS` に登録（行版 `_CHASERS` には登録しない＝行版 extract/mask 不要）** |
| 6 | `fixedpoint/_ingest.py` / `_seed.py` / `_scan.py` | 抽出/投入分離（§6.1）・方式A の worker 内 AST 抽出（§6.2）・found タプル拡張 |
| 7 | `chase.py` | `_AST_CHASERS` 参照の `extract_chase_symbols_tree(language, text, lineno)` dispatcher 追加（行言語は既存 `_CHASERS` 経路） |
